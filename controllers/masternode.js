...
const getMasterNode = async (req, res, next) => {
  try {
    ...
    let decryptData = {};
    let user = await admin.firestore().collection(process.env.COLLECTION_NAME_USERS).doc(req.user).get().catch(err => {
      throw err
    })
    if (typeof user._fieldsProto === "undefined") return res.status(406).json({ok: false, message: 'non-existent user'});
    // if (user.id !== req.user) return res.status(406).json({ok: false, message: 'you do not have permissions to perform this action'})
...
const masterNodeVoteIn = async (req, res, next) => {
  try {
    ...
    if (user._fieldsProto.mNodeList || user._fieldsProto.mNodeList.arrayValue.values.length > 0) {
      ...
      if (typeof verifyMNInUser === "undefined") return res.status(406).json({ok: false, message: 'masternode not found'})
    }
    let mn = await admin.firestore().collection(process.env.COLLECTION_NAME_MASTERNODES).doc(id).get().catch(err => {
      throw err
    })
    if (typeof mn._fieldsProto === "undefined") return res.status(204).json({ok: false, message: 'MasterNode does not exist'})
