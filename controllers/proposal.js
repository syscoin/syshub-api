const {clientRPC, rpcServices, firebase, admin} = require('../utils/config');
...
const check = async (req, res, next) => {
  try {
    let {type, name, title, description, nPayment, firstEpoch, startEpoch, endEpoch, paymentAddress, paymentAmount, url} = req.body;
    if (!type || !name || !title || !nPayment || !firstEpoch || !startEpoch || !endEpoch || !paymentAddress || !paymentAmount) return res.status(406).json({ok: false, messageL: 'Required fields'});
    if (/\s/.test(name)) return res.status(406).json({ok: false, message: 'Invalid name proposal, name contains invalid characters'});
    let objectProposal = [
      ['proposal', {
        type: Number(type),
        name,
    ...
    let verifyHex = await clientRPC.callRpc('gobject', ['check', hexProposal]).call().catch(err => {
      throw err
    })
    ...
    if (err.message === 'Invalid proposal data, error messages:data exceeds 512 characters;JSON parsing error;') return res.status(400).json({ok: false, message: 'Invalid Proposal'})
    if (err.message === 'Invalid object type, only proposals can be validated') return res.status(400).json({ok: false, message: 'Invalid Proposal'})
    next(err)
  }
}
...
const prepare = async (req, res, next) => {
  try {
    ...
    if (typeof user._fieldsProto === "undefined") return res.status(406).json({ok: false, message: 'usuario inexistente'});
    ...
    if (verifyHex && Object.values(verifyHex)[0] === 'OK') {
      ...
      let command = `gobject prepare ${parentHash} ${revision} ${time} ${dataHex}`;
      let proposalResp = await admin.firestore().collection(process.env.COLLECTION_NAME_PROPOSAL).add({
        type: Number(type),
        name,
        title,
        description,
      ...
      await admin.firestore().doc(`${process.env.COLLECTION_NAME_USERS}/${req.user}`).update('proposalList', proposals)
...
const submit = async (req, res, next) => {
  try {
    ...
    if (!parentHash || !revision || !time || !dataHex || !txId) return res.status(406).json({ok: false, message: 'Required fields'});
    let user = await admin.firestore().collection(process.env.COLLECTION_NAME_USERS).doc(req.user).get();
    if (typeof user._fieldsProto === "undefined") return res.status(406).json({ok: false, message: 'non-existent user'});
    if (user.id !== req.user) return res.status(406).json({ok: false, message: 'you do not have permissions to perform this action'})

    let commandSubmit = `gobject submit ${parentHash} ${revision} ${time} ${dataHex} ${txId}`.trim();

    let verifyHex = await clientRPC.callRpc('gobject', ['check', dataHex]).call().catch(err => {
      throw err
    })
    if (verifyHex && Object.values(verifyHex)[0] === 'OK') {
      await admin.firestore().collection(process.env.COLLECTION_NAME_PROPOSAL).doc(id).update({commandSubmit: commandSubmit}).catch(err => {
        throw err
      })
...
const vote = async (req, res, next) => {
  try {
    ...
    if (!txHash || !txIndex || !governanceHash || !signal || !vote || !time || !signature) return res.status(406).json({ok: false, message: 'Required fields'});
    let user = await admin.firestore().collection(process.env.COLLECTION_NAME_USERS).doc(req.user).get();
    if (typeof user._fieldsProto === "undefined") return res.status(406).json({ok: false, message: 'non-existent user'});
    if (user.id !== req.user) return res.status(406).json({ok: false, message: 'you do not have permissions to perform this action'})
      ...
      if (user._fieldsProto.proposalList) {
        Promise.all(user._fieldsProto.proposalList.arrayValue.values.map(async proposal => {
          return new Promise((resolve, reject) => {
            admin.firestore().collection(process.env.COLLECTION_NAME_PROPOSAL).doc(proposal.stringValue).get().then(resp => {
              resolve(resp)
            }).catch(err => {
              reject(err)
            })
          ...
          if (typeof currentNoCompleteProposal === "undefined") return res.status(204).json({oK: false, message: 'there are no pending proposals'})
  ...
  try {
    ...
    if (user._fieldsProto.proposalList && user._fieldsProto.proposalList.arrayValue.values.length > 0) {
      ...
      if (typeof existProposalsInUser !== "undefined") {
        let {_fieldsProto} = await admin.firestore().collection(process.env.COLLECTION_NAME_PROPOSAL).doc(id).get();
...
const getAllHiddenProposal = async (req, res, next) => {
  try {
    ...
    let data = await admin.firestore().collection(process.env.COLLECTION_PROPOSAL_HIDDEN).get().catch(err => {
      throw err
    })
    ...
    return res.status(200).json({ok: true, hashs})
  } catch (err) {
    next(err)
  }
}
...
const createHiddenProposal = async (req, res, next) => {
  try {
    let {hash} = req.body;
    let isHidden = await admin.firestore().collection(process.env.COLLECTION_PROPOSAL_HIDDEN).where('hash', "==", hash).get().catch(err => {
      throw err
    })
    if (isHidden.size > 0) return res.status(200).json({ok: false, message: 'Proposal already hidden'});

    let proposalHiddenData = {
      hash
    }
    await admin.firestore().collection(process.env.COLLECTION_PROPOSAL_HIDDEN).add(proposalHiddenData).catch(err => {
      throw err
    })
...
const deleteHiddenProposal = async (req, res, next) => {
  try {
    let {hash} = req.params;
    let isHidden = await admin.firestore().collection(process.env.COLLECTION_PROPOSAL_HIDDEN).doc(hash).get().catch(err => {
      throw err
    })
    ...
    await admin.firestore().collection(process.env.COLLECTION_PROPOSAL_HIDDEN).doc(hash).delete().catch(err => {
      throw err
    })
    let hashs = [];
    let data = await admin.firestore().collection(process.env.COLLECTION_PROPOSAL_HIDDEN).get().catch(err => {
...
const updateProposal = async (req, res, next) => {
  try {
    ...
    if (typeof existProposalInUser === "undefined") return res.status(204).json({ok: false, message: 'non-existent proposal'});
    for (const key in data) {
      newData[key] = data[key]
    }
    await admin.firestore().doc(`${process.env.COLLECTION_NAME_PROPOSAL}/${id}`).update(newData).catch(err => {
      throw err
    });

    let proposal = await admin.firestore().doc(`${process.env.COLLECTION_NAME_PROPOSAL}/${id}`).get()
  ...
  try {
    ...
    if (user._fieldsProto.proposalList.arrayValue.values.length > 0) {
      ...
      if (typeof existProposalInUser !== "undefined") {
        ...
        await admin.firestore().doc(`${process.env.COLLECTION_NAME_USERS}/${req.user}`).update('proposalList', proposals).catch(err => {
          throw err
        })
        await admin.firestore().doc(`${process.env.COLLECTION_NAME_PROPOSAL}/${id}`).delete().catch(err => {
          throw err
        })
