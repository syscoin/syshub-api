...
const prepareMn = (req, res, next) => {
  try {
    ...
    if (
      collateralHash === undefined
      || collateralHash.length === 0
      || collateralIndex === undefined
      || collateralIndex.length === 0
      || ipAddress === undefined
      || ipAddress.length === 0
      || ownerKeyAddr === undefined
      || ownerKeyAddr.length === 0
      || operatorPubKey === undefined
      || operatorPubKey.length === 0
      || votingKeyAddr === undefined
      || votingKeyAddr.length === 0
      || operatorReward === undefined
      || operatorReward.length === 0
    ...
    if (
      (typeof collateralIndex !== 'number'
        && !Number.isFinite(collateralIndex))
      ...
      return res
        ...
        .json({
          ok: false,
          message: 'invalid collateral Index or operator reward',
        })
    }
